...
class BlobListFileStrategy(ListFileStrategy):
    """
    Concrete strategy for listing files that are located in Azure Blob Storage, with MD5 checking using blobs.
    """
    ...
    async def list(self) -> AsyncGenerator[File, None]:
        ...
        async for blob_name in self.list_paths():
            if not await self.check_md5(container_client, blob_name):
    ...
    async def check_md5(self, container_client, blob_name: str) -> bool:
        blob_client = container_client.get_blob_client(blob_name)
        stream = await blob_client.download_blob()
        data = await stream.readall()
        existing_hash = hashlib.md5(data).hexdigest()

        md5_blob_name = f"{blob_name}.md5"
        md5_blob_client = container_client.get_blob_client(md5_blob_name)
        try:
            md5_stream = await md5_blob_client.download_blob()
            stored_hash = (await md5_stream.readall()).decode("utf-8").strip()
        except Exception:
            stored_hash = None
        ...
        # Write the hash to the .md5 blob
        await md5_blob_client.upload_blob(existing_hash, overwrite=True)
        return False
